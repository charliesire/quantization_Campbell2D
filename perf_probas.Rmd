...
This notebook evaluates the precision of the metamodel on the evaluation of the probabilities
...
The method is to : 
  ...
  - Compute $(\Gamm^r)_{r \in \{1,\dots,100\}}$ from the $\Gamm^{\star}$ obtained by applying the lloyd algorithm with 10^6 maps (notebook lloyd_true.Rmd). A random change between 0 and $20\%$ is apply to $\Gamm^{\star}$
  - Compute $\forall r \in \{1,\dots,100\}, \erreurprobarelative^{pred}_{n}(\Gamm^{r},j) = \frac{\mid  \hat{P}^{true}_{n}(\Gamm^{r},j) - \hat{P}^{pred}_{n}(\Gamm^{r},j) \mid}{\hat{P}^{true}_{n}(\Gamm^{r},j)}$
  
  with $\hat{P}^{true}_{n}(\Gamm,j) = \frac{1}{n} \sum^{n}_{k=1} \mathbb{1}_{\yinclust{\Ytrue(\tilde{X}^{(k)})}{\Gamm}{j} }\frac{f_{X}(\tilde{X}^{(k)})}{\biais(\tilde{X}^{(k)}}$, $\hat{P}^{pred}_{n}(\Gamm,j) = \frac{1}{n} \sum^{n}_{k=1} \mathbb{1}_{\yinclust{\Ypred(\tilde{X}^{(k)})}{\Gamm}{j} }\frac{f_{X}(\tilde{X}^{(k)})}{\biais(\tilde{X}^{(k)}}$ and $n= 10^5$
...
## Get $\Gamm^{\star}$ from the notebook lloyd_true.Rmd
...
For all $\tilde{K}$ and $n_{pc}$, we compute : $\forall j \in {1,\dots, 5},\erreurprobarelative^{pred}_{10^4}(\Gamm^{\star},j)$. 
...
Compute the relative error and display the distribution 
