...
#'
#' @export
predict.km_Fpca2d <-function(object,newdata,type,compute = TRUE,fpca = NULL,...){
  ...
  # fpca object
  if(is.null(fpca)){fpca <-attr(object,"fpca")}
  ...
  p_PC <-foreach(i = 1:nPC) %do% {
    predict(object=object[[i]],newdata=newdata,type=type,
            compute = compute,...)
  }# end p.PC
  ...
  # prediction
  #%%%%%%%%%%%%%
  p_mean <-sapply(1:nPC, function(i){p_PC[[i]]$mean}) 
  hatY <- inverse_Fpca2d(p_mean,fpca)
  res$mean <-hatY
