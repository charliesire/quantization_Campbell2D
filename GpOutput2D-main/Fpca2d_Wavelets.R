...
Fpca2d.Wavelets_new<-function(x,
                              wf,J,boundary="periodic", # wavelet parameters
                              p=1,idx_pca, # number of coefficients for PCA
                              center=TRUE,scale.=FALSE,ncoeff = NULL,...){ # PCA parameters
  if(length(dim(x)) == 2){x = array(x, dim = c(dim(x)[1], dim(x)[2], 1))}
  d<-dim(x) # dimensions of x
